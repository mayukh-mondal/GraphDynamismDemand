"""
Load trained GAT and GATv2 checkpoints, run inference on ogbn-mag, extract attention weights, and compute distance metrics between the two models' attention distributions.

ogbn-mag is a heterogeneous graph with four node types (paper, author, institution,
field_of_study) and four edge types. Only paper nodes carry features (128-dim
PaperNode2Vec embeddings, same as ogbn-arxiv). This script extracts the homogeneous
paper→paper "cites" subgraph and runs the standard attention comparison on it.

  paper nodes : 736,389
  cites edges :  ~5.4M  (directed, same structure as ogbn-arxiv)

Because the paper-cites-paper subgraph fits entirely in GPU memory, sampling is
optional. The default (--sample_nodes 0) uses the full subgraph.
...
Notes on feature dimension:
  ogbn-mag paper nodes : 128-dim PaperNode2Vec embeddings (identical to ogbn-arxiv).
  in_channels=128 matches the arxiv checkpoint directly — no padding required.

Notes on graph structure:
  The paper-cites-paper subgraph is directed (like ogbn-arxiv).

Notes on data loading:
  Uses OGB's numpy interface (NodePropPredDataset) rather than PygNodePropPredDataset
  to avoid HeteroData key-naming issues across torch-geometric versions. Features and
  edge index are constructed from numpy arrays directly.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("logs/compare_attention_mag.log"),
    ],
)
...
def parse_args():
    ...
    p.add_argument("--sample_nodes", type=int, default=0,
                   help="Random induced subgraph over paper nodes "
                        "(0 = full paper-cites-paper subgraph, recommended — fits on a T4 GPU).")
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    # Use the numpy interface to avoid HeteroData key-naming issues across PyG versions.
    dataset    = NodePropPredDataset(name="ogbn-mag", root=args.data_dir)
    graph, _   = dataset[0]

    # Extract the paper-cites-paper homogeneous subgraph.
    # Only paper nodes carry features (128-dim); other node types are discarded.
    x          = torch.from_numpy(graph['node_feat_dict']['paper']).float()       # [736389, 128]
    ei_np      = graph['edge_index_dict'][('paper', 'cites', 'paper')]            # numpy [2, ~5.4M]
    ...
    if args.sample_nodes > 0 and args.sample_nodes < num_nodes:
        log.info(f"Sampling {args.sample_nodes:,} random paper nodes (induced subgraph) …")
        perm      = torch.randperm(num_nodes)[:args.sample_nodes]
        node_mask = torch.zeros(num_nodes, dtype=torch.bool)
        node_mask[perm] = True
        sub_ei, _ = subgraph(
            perm, edge_index, relabel_nodes=True,
        ...
        log.info("Using full paper-cites-paper subgraph (no sampling) …")
        x          = x.to(device)
        edge_index = edge_index.to(device)

    log.info(f"  Nodes : {num_nodes:,}")
    log.info(f"  Edges : {edge_index.size(1):,}  (directed)")
    ...
    log.info(f"  Dataset : ogbn-mag (paper-cites-paper)  |  Layer : {layer}")
    log.info("═" * 60)
