"""
Load trained GAT and GATv2 checkpoints, run inference on ogbn-proteins, extract attention weights, and compute distance metrics between the two models' attention distributions.
...
Notes on feature dimension:
  ogbn-proteins has no node features; the 8-dim edge features (experimental evidence types) are
  mean-aggregated per target node to produce 8-dim node features, then zero-padded to 128 to
  match the ogbn-arxiv checkpoint's in_channels.

Notes on graph structure:
  ogbn-proteins is an undirected graph; the OGB PyG loader already includes both directions,
  so no reverse-edge augmentation is needed.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("logs/compare_attention_proteins.log"),
    ],
)
...
if __name__ == "__main__":
    ...
    log.info("Loading ogbn-proteins …")
    dataset   = PygNodePropPredDataset(name="ogbn-proteins", root=args.data_dir)
    data      = dataset[0]

    # ogbn-proteins is undirected; the PyG loader already includes both directions.
    num_nodes = data.num_nodes

    # ogbn-proteins has no node features; mean-aggregate 8-dim edge features per target node,
    # then zero-pad to 128 to match the ogbn-arxiv checkpoint's in_channels.
    ...
    log.info(f"  Dataset : ogbn-proteins  |  Layer : {layer}")
    log.info("═" * 60)
