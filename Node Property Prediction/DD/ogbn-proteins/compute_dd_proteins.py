"""
compute_dd_proteins.py — compute DD_proxy(G) from trained GAT and GATv2 checkpoints
                         evaluated on ogbn-proteins.
...
on ogbn-proteins (132,534 nodes, 8-dim edge features, protein-protein interaction graph).
ogbn-proteins has no node features; the 8-dim edge attributes are mean-aggregated per target node
to produce 8-dim pseudo-node features, then zero-padded to 128-dim to match the frozen checkpoint's
in_channels.  DD_proxy remains valid because it is a relative GAT-vs-GATv2 comparison; both models
see identical padded features.

Graph structure
---------------
ogbn-proteins is undirected and the OGB PyG loader already includes edges in both directions.
No reverse-edge augmentation is applied (unlike ogbn-products which is directed).

Sample size
-----------
default sample_nodes = 30_000.
GATv2Conv materialises [E, heads×out_ch] tensors (x_i and x_j) per layer during the
attention computation.  Edge count scales as (k/132534)², so the per-tensor peak is:
  30k nodes  → ~4.1M edges  → x_i ≈ 4.2 GB  → total peak ~9 GB   (safe on T4)
  50k nodes  → ~11.3M edges → x_i ≈ 11.6 GB → total peak ~24 GB  (OOM on T4)
Use sample_nodes=0 only if you have ≥40 GB VRAM (A100-80G or equivalent).
...
- ogbn-proteins node features are constructed by mean-aggregating edge_attr (8-dim)
  over incoming edges per node, then zero-padding to 128-dim.
"""
...
def parse_args():
    ...
    p.add_argument("--dataset",      default="ogbn-proteins")
    p.add_argument("--data_dir",     default="./data")
    p.add_argument("--sample_nodes", type=int, default=30_000,
                   help="Random induced subgraph size; 0 = full graph (requires ≥40 GB VRAM)")
    p.add_argument("--seed",         type=int, default=42)
    p.add_argument("--device",       default="auto")
    p.add_argument("--min_deg",      type=int, default=3)
    p.add_argument("--out",          default="Node Property Prediction\\DD\\ogbn-proteins\\dd_results\\dd_proteins.pt")
...
def load_graph(dataset_name: str, data_dir: str,
    ...
    """Load ogbn-proteins, build node features from edge attributes, optionally subsample."""
    import gc
    ...
    # ogbn-proteins has no node features; mean-aggregate 8-dim edge_attr per target node
    # to produce pseudo-node features, then zero-pad to 128 to match the checkpoint's in_channels.
    log.info(f"  Building node features from edge_attr (mean-agg per target node) …")
    x = scatter(data.edge_attr, data.edge_index[1], dim=0,
                dim_size=num_nodes, reduce='mean')   # [N, 8]
    x = F.pad(x, (0, 128 - x.size(1)))              # [N, 128]
    log.info(f"  Feature dim after padding: {x.size(1)}")

    # ogbn-proteins is undirected; the OGB PyG loader already provides both directions.
    # No reverse-edge augmentation needed.
...
def save_plots(dd: Tensor, rho_bar: Tensor, deg: Tensor,
    ...
    axes[1].set_title("Per-node cross-model Spearman rho")
    axes[1].legend()
...
if __name__ == "__main__":
    ...
    log_path = Path("Node Property Prediction/DD/ogbn-proteins/compute_dd_proteins.log")
