...
compute_dd.py — compute DD_proxy(G) from trained GAT and GATv2 checkpoints.
    ...
    rho_bar_i   = sum_l sum_h rho_i^(l,h) / sum_l H_l        (head-weighted, cross-layer)
...
- Degree is computed from the attention edge_index, which includes self-loops
  added by GATConv/GATv2Conv.  min_deg=3 therefore means >= 2 real neighbours.
...
def parse_args():
    ...
    p.add_argument("--gat_ckpt",     default="checkpoints/best_gat_arxiv.pt")
    p.add_argument("--gatv2_ckpt",   default="checkpoints/best_gatv2_arxiv.pt")
    ...
    p.add_argument("--out",          default="dd_results/dd_products.pt")
...
def compute_dd_proxy(gat_model, gatv2_model,
    ...
    rho_bar  : Tensor [N]    per-node head-weighted cross-layer Spearman rho
    deg      : Tensor [N]    degree including self-loops
    """
    _, gat_attn   = gat_model(x, edge_index)
    _, gatv2_attn = gatv2_model(x, edge_index)

    # Degree from layer-0 attention edge_index (self-loops already added by GATConv)
    src_0 = gat_attn[0][0][0].cpu()   # gat_attn[0] = (ei, alpha), ei[0] = src
    deg   = scatter(torch.ones(src_0.shape[0]), src_0,
                    dim_size=num_nodes, reduce='sum').long()   # [N]

    weighted_rho = torch.zeros(num_nodes)
    total_heads  = 0
    ...
    for l in range(num_layers):
        ...
        rho_l = per_node_spearman(alpha_gat_l, alpha_gatv2_l, src_l, num_nodes)

        weighted_rho = weighted_rho + H_l * rho_l.nan_to_num(0.0)
        total_heads  += H_l

        # Per-layer stats (restricted to valid, included nodes)
        mask = (deg >= min_deg)
        r    = rho_l[mask]
        log.info(f"  Layer {l}  H={H_l}  "
                 f"mean rho={r.mean():.4f}  median={r.median():.4f}  std={r.std():.4f}")

    rho_bar = weighted_rho / total_heads          # head-weighted mean across layers [N]
    rho_bar[deg < min_deg] = float('nan')         # exclude low-degree nodes

    dd = 1.0 - rho_bar.clamp(min=0.0)            # dd(i) in [0, 1]; NaN where excluded
    dd[deg < min_deg] = float('nan')

    # Degree-weighted mean over valid nodes
    valid = ~dd.isnan()
    w     = deg.float() * valid.float()
    DD    = (dd.nan_to_num(0.0) * w).sum().item() / w.sum().item()

    return DD, dd, rho_bar, deg
...
if __name__ == "__main__":
    ...
    log_path = Path("logs/compute_dd.log")
